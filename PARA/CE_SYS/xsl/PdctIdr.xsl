...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:Doc="urn:com.cs.tsu">
    ...
    <xsl:include href="xsl_comp/PdctIdr_addCheckbox.xsl"/>
						...
						<tr>
							...
							<td colspan="10">
								<table width="99%" border="0" cellpadding="0" cellspacing="0">
									...
									<xsl:for-each select="Doc:PdctIdr">
										<xsl:variable name="PdctIdr" select="."/>
										<xsl:call-template name="PdctIdr">
											<xsl:with-param name="PdctIdr" select="$PdctIdr"/>
											<xsl:with-param name="PdctIdr_parentName">
												<xsl:number value="position()"/>_<xsl:value-of select="'Doc:PdctIdr'"/>
											</xsl:with-param>
										</xsl:call-template>
									</xsl:for-each>
								</table>
