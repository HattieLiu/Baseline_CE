...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template name="BICIdentification1">
		...
		<xsl:variable name="submissnIDDesc">
			<xsl:call-template name="getFldDesc1">
				<xsl:with-param name="FldId" select="'SubmissnId'"/>
			</xsl:call-template>
		</xsl:variable>
		<tr>
			<xsl:element name="TD">
				...
				<a>
					...
					<xsl:attribute name="onClick">Swap('<xsl:value-of select="$BICIdentification1_pname"/>_<xsl:value-of select="$BICIdentification1_name"/>')</xsl:attribute>
		...
		<tr>
			<xsl:element name="TD">
				...
				<xsl:element name="DIV">
					<xsl:attribute name="id"><xsl:value-of select="$BICIdentification1_pname"/>_<xsl:value-of select="$BICIdentification1_name"/></xsl:attribute>
						...
						<tr>
							<xsl:element name="TD">
								...
								<xsl:element name="INPUT">
									...
									<xsl:attribute name="value"><xsl:value-of select="$BICIdentification1/BIC"/></xsl:attribute>
								</xsl:element>
							</xsl:element>
						</tr>
					</table>
