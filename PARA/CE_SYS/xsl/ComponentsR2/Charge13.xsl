		...
		<tr>
			<xsl:element name="TD">
				...
				<a>
					...
					<xsl:attribute name="onClick">Swap('<xsl:value-of select="$Charge13_pname"/>_<xsl:value-of select="$Charge13_name"/>')</xsl:attribute>
		...
		<tr>
			<xsl:element name="TD">
				...
				<xsl:element name="DIV">
					<xsl:attribute name="id"><xsl:value-of select="$Charge13_pname"/>_<xsl:value-of select="$Charge13_name"/></xsl:attribute>
						...
						<tr>
							<xsl:element name="TD">
								...
								<xsl:element name="INPUT">
									...
									<xsl:attribute name="value"><xsl:value-of select="$Charge13/Tp"/></xsl:attribute>
								</xsl:element>
							</xsl:element>
						</tr>
						<xsl:for-each select="$Charge13/Chrgs">
							...
							<xsl:call-template name="ChargesDetails2">
								...
								<xsl:with-param name="ChargesDetails2_parent">
									<xsl:number value="position()"/>_<xsl:value-of select="$Charge13_pname"/>_<xsl:value-of select="$Charge13_name"/>
								</xsl:with-param>
							</xsl:call-template>
						</xsl:for-each>
					</table>
