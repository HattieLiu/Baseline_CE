...
<!--  *********** point <PercentageTolerance1> start ********** -->
	<xsl:template name="PercentageTolerance1">
		...
		<xsl:variable name="PlusPctDesc">
			<xsl:call-template name="getFldDesc1">
				<xsl:with-param name="FldId" select="'PlusPct'"/>
			</xsl:call-template>
		</xsl:variable>
		<xsl:variable name="MnsPctDesc">
			<xsl:call-template name="getFldDesc1">
				<xsl:with-param name="FldId" select="'MnsPct'"/>
			</xsl:call-template>
		</xsl:variable>
		<tr>
			<xsl:element name="TD">
						...
						<tr>
							<xsl:element name="TD">
								...
								<xsl:element name="INPUT">
									...
									<xsl:attribute name="value"><xsl:value-of select="$PercentageTolerance1/PlusPct"/></xsl:attribute>
								</xsl:element>
							</xsl:element>
							<xsl:element name="TD">
								<xsl:attribute name="style">width:30%;FONT-FAMILY: Verdana, Arial;FONT-SIZE: 11px;</xsl:attribute>
								...
								<xsl:element name="INPUT">
									...
									<xsl:attribute name="length">20</xsl:attribute>
									<xsl:attribute name="maxlength">20</xsl:attribute>
									<xsl:attribute name="value"><xsl:value-of select="$PercentageTolerance1/MnsPct"/></xsl:attribute>
								</xsl:element>
							</xsl:element>
						</tr>
					</table>
	...
	<!--  *********** point <PercentageTolerance1> end ********** -->
</xsl:stylesheet>
