...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:Doc="urn:com.cs.tsu">
    ...
    <xsl:include href="xsl_comp/Tax014_addCheckbox.xsl"/>
								...
								<table width="99%" border="0" cellpadding="0" cellspacing="0">
									...
									<xsl:for-each select="Doc:Tax">
										...
										<xsl:call-template name="Tax">
											<xsl:with-param name="Tax" select="$Tax"/>
											<xsl:with-param name="Tax_parentName">
												<xsl:number value="position()"/>_<xsl:value-of select="'Doc:Tax'"/>
											</xsl:with-param>
										</xsl:call-template>
									</xsl:for-each>
								</table>
