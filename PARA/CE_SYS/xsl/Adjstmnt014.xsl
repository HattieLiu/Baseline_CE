...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:Doc="urn:com.cs.tsu">
    ...
    <xsl:include href="xsl_comp/Adjstmnt014_addCheckbox.xsl"/>
							...
							<td colspan="10">
								<table width="99%" border="0" cellpadding="0" cellspacing="0">
									...
									<xsl:for-each select="Doc:Adjstmnt">
										<xsl:variable name="Adjstmnt" select="."/>
										<xsl:call-template name="Adjstmnt">
											<xsl:with-param name="Adjstmnt" select="$Adjstmnt"/>
											<xsl:with-param name="Adjstmnt_parentName">
												<xsl:number value="position()"/>_<xsl:value-of select="'Doc:Adjstmnt'"/>
											</xsl:with-param>
										</xsl:call-template>
									</xsl:for-each>
								</table>
