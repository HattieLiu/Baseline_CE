...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:Doc="urn:com.cs.tsu">
    ...
    <xsl:include href="xsl_comp/UpdtdElmt_addCheckbox.xsl"/>
							...
							<td colspan="10">
								<table width="99%" border="0" cellpadding="0" cellspacing="0">
									...
									<xsl:for-each select="Doc:UpdtdElmt">
										<xsl:variable name="UpdtdElmt" select="."/>
										<xsl:call-template name="UpdtdElmt">
											<xsl:with-param name="UpdtdElmt" select="$UpdtdElmt"/>
											<xsl:with-param name="UpdtdElmt_parentName">
												<xsl:number value="position()"/>_<xsl:value-of select="'Doc:UpdtdElmt'"/>
											</xsl:with-param>
										</xsl:call-template>
									</xsl:for-each>
								</table>
