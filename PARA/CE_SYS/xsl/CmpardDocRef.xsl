...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:Doc="urn:com.cs.tsu">
	...
	<xsl:include href="xsl_comp/CmpardDocRef_addCheckbox.xsl"/>
							...
							<td colspan="10">
								<table width="99%" border="0" cellpadding="0" cellspacing="0">
									...
									<xsl:for-each select="Doc:CmpardDocRef">
										<xsl:variable name="CmpardDocRef" select="."/>
										<xsl:call-template name="CmpardDocRef">
											<xsl:with-param name="CmpardDocRef" select="$CmpardDocRef"/>
											<xsl:with-param name="CmpardDocRef_parentName">
												<xsl:number value="position()"/>_<xsl:value-of select="'Doc:CmpardDocRef'"/>
											</xsl:with-param>
										</xsl:call-template>
									</xsl:for-each>
								</table>
