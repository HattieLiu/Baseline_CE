...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:Doc="urn:com.cs.tsu">
   ...
   <xsl:include href="TransportDetails1.xsl"/>
	...
	<xsl:template name="TransportDataSet2">
	    <xsl:param name="stmt1" select="Doc:NewTrnsprtDataSet"/>
		<xsl:param name="TransportDataSet2"/>
		<xsl:param name="TransportDataSet2_parent"/>
		<xsl:variable name="TransportDataSet2_name" select="name($TransportDataSet2)"/>
		<xsl:variable name="TransportDataSet2_pname" select="$TransportDataSet2_parent"/>
		...
		<tr>
			<xsl:element name="TD">
				...
				<a>
					...
					<xsl:attribute name="onClick">Swap('<xsl:value-of select="$TransportDataSet2_pname"/>_<xsl:value-of select="$TransportDataSet2_name"/>')</xsl:attribute>
		...
		<tr>
			<xsl:element name="TD">
				...
				<xsl:element name="DIV">
					<xsl:attribute name="id"><xsl:value-of select="$TransportDataSet2_pname"/>_<xsl:value-of select="$TransportDataSet2_name"/></xsl:attribute>
					...
					<table width="95%" border="0" cellpadding="0" cellspacing="0" align="right">
						...
						<xsl:for-each select="$TransportDataSet2/Doc:TrnsprtInf">
						<xsl:variable name="TransportDetails1" select="."/>
						<xsl:call-template name="TransportDetails1">
							<xsl:with-param name="TransportDetails1" select="$TransportDetails1"/>
							<xsl:with-param name="TransportDetails1_parent">
