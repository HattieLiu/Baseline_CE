...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:Doc="urn:com.cs.tsu">
    ...
    <xsl:include href="DocumentIdentification7.xsl"/>
     ...
     <xsl:include href="RequiredSubmission1.xsl"/>
<!--  *********** point <Baseline2> Start ********** -->
	<xsl:template name="Baseline2">
		<xsl:param name="Baseline2"/>
		<xsl:param name="stmt" select="Doc:BaseIn"/>
			...
			<xsl:element name="TD">
				...
				<xsl:element name="DIV">
					...
					<table width="100%" border="0" cellpadding="0" cellspacing="0" align="right">
						...
						<xsl:for-each select="$Baseline2/Doc:PmtTerms">
						...
						<xsl:call-template name="PaymentTerms1">
							...
							<xsl:with-param name="PaymentTerms1_parent">
							      <xsl:number value="position()"/>_<xsl:value-of select="$Baseline2_name"/>
							</xsl:with-param>
						</xsl:call-template>
						</xsl:for-each>
						
						<xsl:variable name="SettlementTerms1" select="$Baseline2/Doc:SttlmTerms"/>
						<xsl:call-template name="SettlementTerms1">
							<xsl:with-param name="SettlementTerms1" select="$SettlementTerms1"/>
						</xsl:call-template>
						
						<xsl:variable name="RequiredSubmission1" select="$Baseline2/Doc:DataSetReqrd"/>
