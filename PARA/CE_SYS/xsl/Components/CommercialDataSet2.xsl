...
<!--  *********** point <CommercialDataSet2> Start ********** -->
	<xsl:template name="CommercialDataSet2">
		...
		<xsl:param name="stmt1" select="ComrclDataSet"/>
			...
			<xsl:element name="TD">
				...
				<xsl:element name="DIV">
					...
					<table width="95%" border="0" cellpadding="0" cellspacing="0" align="right">
						...
						<xsl:for-each select="$CommercialDataSet2/Doc:ComrclDocRef">
						...
						<xsl:for-each select="$CommercialDataSet2/Doc:Goods">
						<xsl:variable name="LineItem4" select="."/>
						<xsl:call-template name="LineItem4">
							<xsl:with-param name="LineItem4" select="$LineItem4"/>
							<xsl:with-param name="Line_FullName">
								<xsl:value-of select="substring($CommercialDataSet2_name,5)"/>
							</xsl:with-param>
							<xsl:with-param name="LineItem4_parent">
							       <xsl:number value="position()"/>_<xsl:value-of select="$stmt1"/>
 							</xsl:with-param>
						</xsl:call-template>
						</xsl:for-each>
						<xsl:for-each select="$CommercialDataSet2/Doc:PmtTerms">
						...
						<xsl:with-param name="PaymentTerms1_parent">
						     <xsl:number value="position()"/>_<xsl:value-of select="$CommercialDataSet2_name"/>
						</xsl:with-param>
						</xsl:call-template>
						</xsl:for-each>
