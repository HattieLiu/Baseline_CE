		...
		<tr>
			<xsl:element name="TD">
				...
				<a>
					...
					<xsl:for-each select="$SingleTransport2/Doc:TrnsprtByAir">
						<xsl:if test="Doc:AirCrrierNm != '' ">
							<xsl:attribute name="onClick">Swap('<xsl:value-of select="$SingleTransport2_pname"/>_<xsl:value-of select="$SingleTransport2_name"/>')</xsl:attribute>
						</xsl:if>
					</xsl:for-each>
		...
		<tr>
			<xsl:element name="TD">
				...
				<xsl:element name="DIV">
					<xsl:attribute name="id"><xsl:value-of select="$SingleTransport2_pname"/>_<xsl:value-of select="$SingleTransport2_name"/></xsl:attribute>
					<xsl:attribute name="style">DISPLAY:none;overflow: hidden; border: 0px none #000000;width:95%;</xsl:attribute>
					<table width="95%" border="0" cellpadding="0" cellspacing="0" align="right">
						<tr>
							<td/>
						</tr>
						<xsl:for-each select="$SingleTransport2/Doc:PurchsOrdrRef">
						...
						<xsl:for-each select="$SingleTransport2/Doc:TrnsprtBySea">
						<xsl:variable name="TransportBySea2" select="."/>
						<xsl:call-template name="TransportBySea2">
							<xsl:with-param name="TransportBySea2" select="$TransportBySea2"/>
							<xsl:with-param name="TransportBySea2_parent">
							     <xsl:number value="position()"/>_<xsl:value-of select="$stmt2"/>
							</xsl:with-param>
						</xsl:call-template>
						</xsl:for-each>
						<xsl:for-each select="$SingleTransport2/Doc:TrnsprtByRoad">
						...
						<xsl:for-each select="$SingleTransport2/Doc:TrnsprtByRail">
