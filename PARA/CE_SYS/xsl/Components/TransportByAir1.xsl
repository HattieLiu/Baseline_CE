...
<!--  *********** point <TransportByAir1> start ********** -->
	<xsl:template name="TransportByAir1">
		...
		<xsl:param name="TransportByAir1_parent"/>
		<xsl:param name="TrnsprtByAir_FullName"/>
		<xsl:param name="stmt3" select="Doc:DprtureAirprt"/>
		<xsl:variable name="TransportByAir1_name" select="name($TransportByAir1)"/>
		<xsl:variable name="TransportByAir1_pname" select="$TransportByAir1_parent"/>
		...
		<tr>
			<xsl:element name="TD">
				...
				<a>
					...
					<xsl:attribute name="onClick">Swap('<xsl:value-of select="$TransportByAir1_pname"/>_<xsl:value-of select="$TrnsprtByAir_FullName_fox"/>_<xsl:value-of select="$TransportByAir1_name"/>')</xsl:attribute>
		...
		<tr>
			<xsl:element name="TD">
				...
				<xsl:element name="DIV">
					...
					<xsl:attribute name="id"><xsl:value-of select="$TransportByAir1_pname"/>_<xsl:value-of select="$TrnsprtByAir_FullName_fox"/>_<xsl:value-of select="$TransportByAir1_name"/></xsl:attribute>
					   ...
					   <xsl:for-each select="$TransportByAir1/Doc:DprtureAirprt">
						...
						<xsl:call-template name="AirportName1Choice">
							...
							<xsl:with-param name="AirportName1Choice_parent">
							    <xsl:value-of select="$TransportByAir1_pname"/>_<xsl:value-of select="$TrnsprtByAir_FullName_fox"/>_<xsl:value-of select="$TransportByAir1_name"/>
							</xsl:with-param>
						</xsl:call-template>
						</xsl:for-each>
