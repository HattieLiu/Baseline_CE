...
<!--  *********** point <LineItem5> Start ********** -->
	<xsl:template name="LineItem5">
		...
		<xsl:param name="stmt2" select="Doc:Goods"/>
			...
			<xsl:element name="TD">
				...
				<xsl:element name="DIV">
					...
					<table width="100%" border="0" cellpadding="0" cellspacing="0" align="left">
						...
						<xsl:for-each select="$LineItem5/Doc:LineItmDtls">
						<xsl:variable name="LineItemDetails4" select="."/>
						<xsl:call-template name="LineItemDetails4">
							...
							<xsl:with-param name="LineItemDetails4_parent">
							     <xsl:number value="position()"/>_<xsl:value-of select="$Line1_FullName_fox"/>_<xsl:value-of select="$LineItem5_name"/>
							</xsl:with-param>
						</xsl:call-template>
						</xsl:for-each>
						...
						<xsl:for-each select="$LineItem5/Doc:Tax">
						...
						<xsl:for-each select="$LineItem5/Doc:BuyrDfndInf">
						...
						<xsl:for-each select="$LineItem5/Doc:SellrDfndInf">
