...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:Doc="urn:com.cs.tsu">
  ...
  <xsl:include href="Components/TransactionReportItems1.xsl"/>
	<xsl:template match="Doc:Document/Doc:tsmt.041.001.02">
	     <xsl:param name="stmt" select="Doc:tsmt.041.001.02"/>
		<html>
			<head>
				...
				<table width="99%" border="0" cellpadding="0" cellspacing="0">
					<tr>
						<xsl:element name="TD">
							...
							<xsl:text>TransactionReport: (tsmt.041.001.02)</xsl:text>
						</xsl:element>
					</tr>
					<tr>
						<xsl:element name="TD">
					...
					<tr>
						...
						<td colspan="10">
							<table width="99%" border="0" cellpadding="0" cellspacing="0">
								...
								<!-- RptdItms -->
								<xsl:for-each select="Doc:RptdItms">
								<xsl:variable name="TransactionReportItems1" select="."/>
								<xsl:call-template name="TransactionReportItems1">
									<xsl:with-param name="TransactionReportItems1" select="$TransactionReportItems1"/>
									<xsl:with-param name="TransactionReportItems1_parent">
