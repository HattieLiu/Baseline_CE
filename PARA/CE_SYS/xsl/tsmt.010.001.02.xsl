...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:Doc="urn:com.cs.tsu">
	...
	<xsl:include href="Components/Limit1.xsl"/>
	<xsl:include href="Components/DocumentIdentification4.xsl"/>
	<xsl:include href="Components/MisMatchReport2.xsl"/>
	<xsl:include href="Components/PendingActivity1.xsl"/>
	
	<xsl:template match="Doc:Document/Doc:tsmt.010.001.02">
	    <xsl:param name="stmt" select="Doc:tsmt.010.001.02"/>
		<html>
			<head>
				...
				<table width="99%" border="0" cellpadding="0" cellspacing="0">
					<tr>
						<xsl:element name="TD">
							...
							<xsl:text>BaselineMatchReport: (tsmt.010.001.02)</xsl:text>
						</xsl:element>
					</tr>
					<tr>
						<xsl:element name="TD">
					...
					<tr>
						...
						<td colspan="10">
							<table width="99%" border="0" cellpadding="0" cellspacing="0">
								...
								<xsl:variable name="Limit1" select="Doc:BaselnEstblishmtTrils"/>
								<xsl:call-template name="Limit1">
									<xsl:with-param name="Limit1" select="$Limit1"/>
								</xsl:call-template>	
								<!-- CmpardDocRef -->
								<xsl:for-each select="Doc:CmpardDocRef">
								<xsl:variable name="DocumentIdentification4" select="."></xsl:variable>
