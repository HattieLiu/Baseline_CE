...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:Doc="urn:com.cs.tsu">
	...
	<xsl:template match="Doc:Document/Doc:tsmt.013.001.01">
		<html>
			<head>
				...
				<table width="99%" border="0" cellpadding="0" cellspacing="0">
					<tr>
						<xsl:element name="TD">
							...
							<xsl:text>DataSetMatchReport: Structure and Contents(tsmt.013.001.01)</xsl:text>
						</xsl:element>
					</tr>
					<tr>
						<xsl:element name="TD">
     ...
     <xsl:when test="*[tsmt.001.001.01]"> -->
								...
								<!-- CmpardDocRef -->
								<xsl:variable name="DocumentIdentification2" select="Doc:CmpardDocRef"/>
								...
								<!-- Rpt -->
								<xsl:variable name="MisMatchReport1" select="Doc:Rpt"/>
								<xsl:call-template name="MisMatchReport1">
									<xsl:with-param name="MisMatchReport1" select="$MisMatchReport1"/>
								</xsl:call-template>
								<!-- ReqForActn -->
