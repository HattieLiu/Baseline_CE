...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:Doc="urn:com.cs.tsu">
	       <xsl:include href="public.xsl"/>
	        ...
	        <xsl:include href="Components/StatusReportItems1.xsl"/>
	<xsl:template match="Doc:Document/Doc:tsmt.037.001.02">
	    <xsl:param name="stmt" select="Doc:tsmt.037.001.02"/>
		<html>
			<head>
				...
				<table width="99%" border="0" cellpadding="0" cellspacing="0">
					<tr>
						<xsl:element name="TD">
							...
							<xsl:text>StatusReport: (tsmt.037.001.02)</xsl:text>
						</xsl:element>
					</tr>
					<tr>
						<xsl:element name="TD">
					...
					<tr>
						...
						<td colspan="10">
							<table width="99%" border="0" cellpadding="0" cellspacing="0">
								...
								<xsl:variable name="StatusReportItems1" select="."/>
								<xsl:call-template name="StatusReportItems1">
									<xsl:with-param name="StatusReportItems1" select="$StatusReportItems1"/>
									<xsl:with-param name="StatusReportItems1_parent">
