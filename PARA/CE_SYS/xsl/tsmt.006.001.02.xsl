...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:Doc="urn:com.cs.tsu">
	...
	<xsl:template match="Doc:Document/Doc:tsmt.006.001.02">
	    <xsl:param name="stmt" select="Doc:tsmt.006.001.02"/>
		<html>
			<head>
				...
				<table width="99%" border="0" cellpadding="0" cellspacing="0">
					<tr>
						<xsl:element name="TD">
							...
							<xsl:text>AmendmentAcceptanceNotification (tsmt.006.001.02)</xsl:text>
						</xsl:element>
					</tr>
					<tr>
						<xsl:element name="TD">
     ...
     <xsl:when test="*[tsmt.001.001.01]"> -->
								<!-- NtfctnId -->
								 <xsl:variable name="MessageIdentification1" select="Doc:NtfctnId"></xsl:variable>
								...
								<!-- DltaRptRef -->
								<xsl:variable name="MessageIdentification1_double" select="Doc:DltaRptRef"></xsl:variable>
								<xsl:call-template name="MessageIdentification1">
									<xsl:with-param name="MessageIdentification1" select="$MessageIdentification1_double"/>
								</xsl:call-template>
								<!-- AccptdAmdmntNb -->
								<xsl:variable name="Count1" select="Doc:AccptdAmdmntNb"></xsl:variable>
