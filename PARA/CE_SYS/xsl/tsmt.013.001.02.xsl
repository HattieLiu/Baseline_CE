...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:Doc="urn:com.cs.tsu">
	...
	<xsl:include href="Components/DocumentIdentification2.xsl"/>
	<xsl:include href="Components/ReportType3.xsl"/>
	<xsl:include href="Components/MisMatchReport2.xsl"/>
	<xsl:include href="Components/PendingActivity1.xsl"/>
	<xsl:template match="Doc:Document/Doc:tsmt.013.001.02">
		<xsl:param name="stmt" select="'Doc:tsmt.013.001.02'"/>
		<html>
			<head>
				...
				<table width="99%" border="0" cellpadding="0" cellspacing="0">
					<tr>
						<xsl:element name="TD">
							...
							<xsl:text>DataSetMatchReport: (tsmt.013.001.02)</xsl:text>
						</xsl:element>
					</tr>
					<tr>
						<xsl:element name="TD">
					...
					<tr>
						...
						<td colspan="10">
							<table width="99%" border="0" cellpadding="0" cellspacing="0">
								...
								<xsl:for-each select="Doc:CmpardDocRef">
 									<xsl:variable name="DocumentIdentification2" select="."/>
									<xsl:call-template name="DocumentIdentification2">
										<xsl:with-param name="DocumentIdentification2" select="$DocumentIdentification2"/>
										<xsl:with-param name="DocumentIdentification2_parent">
											<xsl:number value="position()"/>_<xsl:value-of select="$stmt"/>
										</xsl:with-param>
									</xsl:call-template>
								</xsl:for-each>
								<!-- SubmissnTp -->
								<xsl:variable name="ReportType3" select="Doc:SubmissnTp"/>
								<xsl:call-template name="ReportType3">
									<xsl:with-param name="ReportType3" select="$ReportType3"/>
								</xsl:call-template>
								<!-- Rpt -->
								<xsl:variable name="MisMatchReport2" select="Doc:Rpt"/>
								<xsl:call-template name="MisMatchReport2">
									<xsl:with-param name="MisMatchReport2" select="$MisMatchReport2"/>
								</xsl:call-template>
								<!-- ReqForActn -->
